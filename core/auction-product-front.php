...
class Auction_Product_Front{
	...
	public function auction_section() {
		...
		if( !Functions::is_auction_product( $post->ID ) ) return;
		...
		if( isset( $_SESSION['wauc_'.Functions::get_current_user_id()]['wauc_data']['result'] )) {

			if( $_SESSION['wauc_'.Functions::get_current_user_id()]['wauc_data']['result'] == 'error' ) {
				?>
				<div class="wauc-notice wauc-notice-danger">
					<?php echo $_SESSION['wauc_'.Functions::get_current_user_id()]['wauc_data']['msg'];?>
				</div>
				<?php
			} else if ( $_SESSION['wauc_'.Functions::get_current_user_id()]['wauc_data']['result'] == 'success' ) {
				?>
			...
			unset( $_SESSION['wauc_'.Functions::get_current_user_id()]['wauc_data'] );
		}

		?>
		<form action="<?php echo get_permalink();?>?wauc_bid=true" method="post">
			<?php $status = Functions::get_auction_time_status( $product->get_id() ); ?>
			...
			if ( $status == 'current' ) {
				...
				<input
					name="bidding_price"
					type="number" value="<?php echo Functions::get_product_auction_price( $product->get_id() ); ?>"
					step="<?php echo Functions::get_bid_increament( $product->get_id() ) ?>">
				<p></p>
				<?php
				$ok_to_bid = 0;

				if( !Functions::is_last_bidder( $product->get_id() ) ) {

					if( Functions::is_eligible_to_bid( Functions::get_current_user_id(), $product->get_id() )) {

						$token = Functions::get_token( $product->get_id() );

						if( $token ) {
							if( !Functions::has_user_token( $token ) ) {
						...
						if( Functions::has_buy_now_price($product) ) {
							// show add to cart if has buy now price and the price < auction price
							if( Functions::get_product_auction_price( $product->get_id() ) <= Functions::has_buy_now_price($product) ) {
								echo apply_filters( 'wauc_set_auction_link',
			...
			elseif ( $status == 'processing' ) {
				...
				if( !wc_customer_bought_product( Functions::get_user_email( Functions::get_current_user_id() ), Functions::get_current_user_id(), $product->get_id() ) ) {
					//check if user selected
					if( Functions::is_selected( Functions::get_current_user_id(), $product->get_id() ) ) {
						?>
		...
		<div id="wauc_time_counter">
			<?php
			if( $status == 'future' ) {
				echo Functions::get_auction_remaining_time( strtotime( Functions::get_auction_start_time( $product->get_id() ) ) );
			} elseif( $status == 'current') {
				echo Functions::get_auction_remaining_time( strtotime( Functions::get_auction_end_time( $product->get_id() ) ) );
			} elseif ( $status == 'processing' ) {
	...
	public function token_section() {
		...
		if( !Functions::is_product_token( $product ) ) return;

		if( wc_customer_bought_product( get_userdata( Functions::get_current_user_id() )->user_email, Functions::get_current_user_id(), $product->get_id() ) ) {
			?>
			<div class="wauc-notice wauc-notice-danger">
	...
	function woocommerce_add_to_cart_validation($valid, $product_id){
		...
		if( Functions::is_product_token( $product ) ) {
			if( wc_customer_bought_product( get_userdata( Functions::get_current_user_id() )->user_email, Functions::get_current_user_id(), $product_id ) ) {
				wc_add_notice( __( 'You have already purchased this item !', 'woocommerce' ), 'error' );
				$valid = false;
			}
		}
		//check if it is auction product
		elseif ( Functions::is_auction_product( $product_id ) ) {
			//check if user already purchased
			if( wc_customer_bought_product( Functions::get_user_email( Functions::get_current_user_id() ), Functions::get_current_user_id(), $product_id ) ) {
				wc_add_notice( __( 'Yor have owned this product already !', 'woocommerce' ), 'error' );
				$valid = false;
			} elseif( !Functions::is_selected( Functions::get_current_user_id(), $product_id ) ) {
	...
	public function process_bid_request() {
		...
		if( isset( $_POST['auction_request'] ) && $_POST['auction_request'] && is_numeric( $_POST['auction_request'] ) ) {
			...
			if( Functions::is_eligible_to_own( Functions::get_current_user_id(), $product->get_id() ) ) {
		...
		if( !Functions::is_auction_product( $product ) ) {
			$_SESSION['wauc_'.Functions::get_current_user_id()]['wauc_data'] = array(
				'result' => 'error',
		...
		$latest_bid_data = Functions::get_latest_bid_row( $product->get_id() );


		//check if current user already bid this
		if( isset( $latest_bid_data->userid ) && $latest_bid_data->userid == Functions::get_current_user_id() ) {
			$_SESSION['wauc_'.Functions::get_current_user_id()]['wauc_data'] = array(
				'result' => 'error',
		...
		$current_auction_price =  Functions::get_product_auction_price( $product->get_id() );
		$bid_increament = Functions::get_bid_increament( $product->get_id() );
		...
		//everything is fine
		Functions::log_bid( $product->get_id(), $_POST['bidding_price'], Functions::get_current_user_id() );
		$_SESSION['wauc_'.Functions::get_current_user_id()]['wauc_data'] = array(
			'result' => 'success',
		...
		//notify users
		WAUC_Notification::send_bid_notification_to_bidder( $product, Functions::get_current_user_id() );
		WAUC_Notification::send_bid_notification_to_participants( $product, Functions::get_current_user_id() );
...
function get_price ($price = '', $product ='') {
	if( !$product ) return;
	if( !Functions::is_auction_product( $product ) ) return $price;
	$bid_price = Functions::is_eligible_to_own( Functions::get_current_user_id(), $product->get_id(), true );
	//pri($bid_price);
...
add_action('woocommerce_checkout_update_order_meta',function( $order_id, $posted ) {
	...
	$order->update_meta_data( 'wauc_order_user_id', Functions::get_current_user_id() );
	$order->save();
} , 10, 2);